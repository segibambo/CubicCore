...
using Cubic.Data.Models.AccountViewModels;
using Cubic.Areas.Portal.Controllers;
...
namespace Cubic.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            ILogger<AccountController> logger, IHostingEnvironment env, IHttpContextAccessor httpContext)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (usermodel != null)
                {
                    var claims = new List<Claim>
                                    {
                                        new Claim("Id", usermodel.Id.ToString(), ClaimValueTypes.String),
                                        new Claim("FullName", usermodel.LastName, usermodel.FirstName + " " + usermodel.MiddleName, ClaimValueTypes.String),
                                    };
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var portalUrl = _httpContext.HttpContext.Request.Host;

                var callbackUrl = Url.Action("ResetPassword", "Account", new { userCode = user.Id.ToString(), code = code });
                string reseturl = portalUrl + callbackUrl;

                // var callbackUrl = Url.ResetPasswordCallbackLink(user.Id.ToString(), code, Request.Scheme);
                await _emailSender.SendEmailResetPasswordAsync("F_PASSWORD", user, reseturl);
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<IActionResult> ResetPassword(string userCode, string code = null)
        {
            if (code == null)
            {
                throw new ApplicationException("A code must be supplied for password reset.");
            }
            var user = await _userManager.FindByIdAsync(userCode);
            if (user == null)
            {
                // Don't reveal that the user does not exist
                return RedirectToAction(nameof(ResetPasswordConfirmation));
            }
            var model = new ResetPasswordViewModel { Code = code, userCode= userCode,Email=user.Email };
            return View(model);
        }
        ...
        private IActionResult RedirectToLocal(string returnUrl)
        {
            ...
            {
                return RedirectToAction(nameof(DashboardController.Index), "Dashboard", new { area = "Portal" });
                //return RedirectToAction(nameof(HomeController.Index), "Home");
            }
        }
